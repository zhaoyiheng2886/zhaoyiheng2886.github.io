...
description: >-
  相较于LDM改动的只有两点： 1. 输入怎么传给Transformer: 把latent切成patch，然后拼成序列，**patch
  size**越小，token 数越多，Gflops 也会明显上升（patch size影响巨大）
...
tags:
  ...
  - Transformer
draft: false
legacyPath: /posts/2026/03/scalable-diffusion-models-with-transformers/
---
相较于LDM改动的只有两点：
1. 输入怎么传给Transformer: 把latent切成patch，然后拼成序列，**patch size**越小，token 数越多，Gflops 也会明显上升（patch size影响巨大）

结构图如下：
![文章配图](/images/posts/image-2026-03-24-15-58-52-762Z.png)
